...
import hashlib
import syslog
from gtk._gtk import Border

try:
    from lib.common.colors import color
    from lib.common.constants import ETHERSNIFF_ROOT, STREAM_TIMING_THRESHOLD, STREAM_PACKET_THRESHOLD
...
class Analyzer(Thread):
    ...
    def extract_files(self,stream):
        ...
        with open(os.path.join(ETHERSNIFF_ROOT,"dl",file_name),'wb') as file:
            start = False
            count = 0
        ...
        # Calculate hash
        hash = self.file_hash(os.path.join(ETHERSNIFF_ROOT,"dl", file_name), hashlib.sha256(),blocksize=65536)
        log.debug("SHA256 (extracted file): %s", hash)

    def cleanup_files(self, stream):
        ''' Delete temporary files. '''
        if stream.contains_download == True:
            if os.path.isfile(os.path.join(ETHERSNIFF_ROOT,"dl",stream.file_name)):
                os.remove(os.path.join(ETHERSNIFF_ROOT,"dl",stream.file_name))
                log.debug("File %s removed.", os.path.join(ETHERSNIFF_ROOT,"dl",stream.file_name))
    ...
    def plot_packets_over_time(self,stream):
        ...
        with open(os.path.join(ETHERSNIFF_ROOT,"log",'tcpstream-' + str(stream.identifier) + '-' + t + '.dat'), 'w+') as file:
    ...
    def plot_packets_over_time_interval(self, stream, interval):
        ...
        t = datetime.fromtimestamp(stream.first_timestamp).strftime('%Y-%m-%d_%H-%M-%S')
        with open(os.path.join(ETHERSNIFF_ROOT,"log","tcpstream-" + str(stream.identifier) + "-" + "int" + str(interval) + "-" + t + ".dat"), 'w+') as file:
            file.write("# TCP stream data\n")
    ...
    def statistical_analysis(self, stream, first_span=STREAM_TIMING_THRESHOLD):
        ...
        # Filename
        log_file = os.path.join(ETHERSNIFF_ROOT,"log","statistical-analysis" + ".dat")
        # Date
        t = datetime.fromtimestamp(stream.first_timestamp).strftime('%Y-%m-%d_%H-%M-%S')
        ...
        with open(os.path.join(ETHERSNIFF_ROOT,"log","statistical-analysis" + ".dat"), 'a') as file:
            if self.comment:
                file.write("\n### {0} ### \n".format(self.comment))
                self.comment = None
